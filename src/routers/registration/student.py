...
from src.repository.registration import (get_cities_schools_pairs, user_exists, is_phone_nr_valid,
                                         is_password_valid, create_request, add_user_to_db)
...
# Wyświetlenie szablonu pozwalającego na wpisanie danych ucznia
@router.get('/student/form', response_class=HTMLResponse)
def register_student_root(request: Request, db: Session = Depends(database.get_db)):
...
# Logika stojąca za rejestracją ucznia
...
def register_student(
    ...
    # Sprawdzenie, czy użytkownik o danym numerze telefonu już istnieje
    if user_exists(db, request):
        raise HTTPException(status_code=status.HTTP_409_CONFLICT, detail=f'User already exists.')

    # Dodanie nowego użytkownika
    user_added = add_user_to_db(db=db, user_type=1, request=request)
